...
int funcionSlave(int tid, int N, int nrProcesos) {
    ...
    // Recibir el bloque
    MPI_Scatter(NULL, 0, MPI_DATA_T, &A[N], bloque, MPI_DATA_T, 0, MPI_COMM_WORLD);
    ...
    while (!convergeG ) {
        ...
        converge = porcesamientoSlave(A, B, N, (N/nrProcesos) - (tid==nrProcesos) , tid,data0,nrProcesos);
